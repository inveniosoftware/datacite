...
# Copyright (C) 2026 Observatoire de Paris.
#
...
import pytest
from helpers import TEST_46_JSON_FILES, load_json_path, load_xml_path
from lxml import etree

from datacite.schema46 import dump_etree, tostring, validator
...
@pytest.mark.parametrize("example_json46", TEST_46_JSON_FILES)
def test_example_json_validates(example_json46):
    """Test the example file validates against the JSON schema."""
    example_json = load_json_path(example_json46)
    validator.validate(example_json)


FILE_PAIRS = [
    (
        "data/4.6/datacite-example-dataset-v4.xml",
        "data/4.6/datacite-example-dataset-v4.json",
    ),
    (
        "data/4.6/datacite-example-instrument-v4.xml",
        "data/4.6/datacite-example-instrument-v4.json",
    ),
    (
        "data/4.6/datacite-example-multilingual-v4.xml",
        "data/4.6/datacite-example-multilingual-v4.json",
    ),
    (
        "data/4.6/datacite-example-relateditem1-v4.xml",
        "data/4.6/datacite-example-relateditem1-v4.json",
    ),
    (
        "data/4.6/datacite-example-relateditem2-v4.xml",
        "data/4.6/datacite-example-relateditem2-v4.json",
    ),
    (
        "data/4.6/datacite-example-relateditem3-v4.xml",
        "data/4.6/datacite-example-relateditem3-v4.json",
    ),
    ("data/datacite-v4.6-full-example.xml", "data/datacite-v4.6-full-example.json"),
]


@pytest.mark.parametrize("example_xml46, example_json46", FILE_PAIRS)
def test_json_to_xml(example_xml46, example_json46, xsd46):
    """Test that example XML converts to example JSON."""
    example_xml = load_xml_path(example_xml46)
    example_json = load_json_path(example_json46)
    xsd46.assertValid(etree.XML(example_xml.encode("utf8")))
    xsd46.assertValid(etree.XML(tostring(example_json).encode("utf8")))


def test_json_eq_xml(example_xml_file46, example_json46, xsd46):
    """Test that example XML converts to example JSON."""
    xsd46.assertValid(etree.XML(tostring(example_json46).encode("utf8")))


#
# Field by field tests.
#
def test_identifier(minimal_json46):
    """Test identifier."""
    data = {"doi": "10.1234/foo.bar"}
    validate_json(minimal_json46, data)
...
def test_creators(minimal_json46):
    """Test creators."""
...
def test_titles(minimal_json46):
    """Test titles."""
...
def test_publisher(minimal_json46):
    """Test publisher."""
...
def test_publicationyear(minimal_json46):
...
def test_subjects(minimal_json46):
    """Test subjects."""
...
def test_contributors(minimal_json46):
    """Test contributors."""
...
def test_language(minimal_json46):
    """Test language."""
    data = {"language": "en"}
    validate_json(minimal_json46, data)
    tree = dump_etree(data)
...
def test_resourcetype(minimal_json46):
    """Test resource type."""
    data = {
...
def test_relatedidentifiers(minimal_json46):
...
def test_formats(minimal_json46):
    """Test formats."""
...
def test_version(minimal_json46):
    """Test version."""
    ...
    assert elem.text == "v4.6"


def test_rights(minimal_json46):
    """Test rights."""
...
def test_descriptions(minimal_json46):
    """Test descriptions."""
...
def test_fundingreferences(minimal_json46):
...
def test_geolocations(minimal_json46):
    """Test geolocation."""
...
# Additional tests
#
def test_minimal_xsd(xsd46, minimal_json46):
    """Test that example XML converts to example JSON."""
    validator.validate(minimal_json46)
    xsd46.assertValid(etree.XML(tostring(minimal_json46).encode("utf8")))


def test_minimal_xml(xsd46):
    """Test minimal xml."""
    xml = """
    <resource
    ...
    xsd46.assertValid(etree.XML(xml))
...
def test_empty_arrays(xsd46, minimal_json46, field_name):
    """Test proper behavior with empty lists for certain fields."""
    minimal_json46[field_name] = []
    validator.validate(minimal_json46)
    xsd46.assertValid(etree.XML(tostring(minimal_json46).encode("utf8")))
